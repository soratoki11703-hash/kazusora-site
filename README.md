...
このサイトは静的ファイルのみ（サーバー処理・DB・フォーム・ログインが無い）ため、
侵入経路は「GitHubアカウント」「ドメイン管理画面」の2つに集約される。
コードを直接攻撃される余地はほぼ無い。

実装済みの対策:

- **JSライブラリを自前配信**（`assets/js/vendor/`）。CDNが乗っ取られると
  サイト上で任意コードを実行されるため、外部依存を断った
- **CSP**（`index.html` の meta）。読み込みを許可する場所を限定している。
  GitHub Pages はHTTPヘッダを設定できないので meta で指定
- **Referrer-Policy**。他サイトへ遷移するときURLの詳細を渡さない
- **画像のEXIF除去**。撮影機材・日時・位置情報を落としてある
- **外部リンクに `rel="noopener"`**。タブナビング対策
- `eval` / `innerHTML` を使っていない

**CSPを変更したら必ず全機能を確認すること。** 設定を誤ると無言で壊れる。

運用側で維持すべきこと:

- GitHubアカウントの2段階認証
- お名前.com の2段階認証
- ドメインの**トランスファーロック**（`clientTransferProhibited`）
